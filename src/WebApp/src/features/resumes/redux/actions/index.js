...
const loadUserResumes = () => async (dispatch, getState, extra) => {
  const { api } = extra;
  const { userID } = getState().auth;
  dispatch({ type: actionTypes.ACTION_PROCESSING, payload: true });
  const response = await api.resume.loadUserResumes(userID);
...
const loadResumeByID = resumeID => async (dispatch, getState, extra) => {
  const { api } = extra;
  dispatch({ type: actionTypes.ACTION_PROCESSING, payload: true });
  const response = await api.resume.loadResumeByID(resumeID);
...
const updateResume = (resumeID, resume) => async (dispatch, getState, extra) => {
  ...
  else dispatch(addNotify(response.errorMessage, 'error'));
